#include "StdAfx.h"
#include "MyPlane.h"
#include "resource.h"

CImageList CMyPlane::m_Images;
CMyPlane::~CMyPlane()
{
}
CMyPlane::CMyPlane(void):m_nHorMotion(0),m_nVerMotion(0)
{
	//nn = n;
	m_nWait = 0;
	m_ptPos.x = 260;
	m_ptPos.y = 600;
}
void CMyPlane::SetPointx(int x)
{
	x = m_ptPos.x;
}
int CMyPlane::GetPointx()
{
	return m_ptPos.x;
}
void CMyPlane::SetPointy(int y)
{
	y = m_ptPos.y;
}
int CMyPlane::GetPointy()
{
	return m_ptPos.y;
}
BOOL CMyPlane::Fired()
{
	if(m_nWait==0)
		return TRUE;
	else
		return FALSE;

}
BOOL CMyPlane::LoadImage()
{
	CGameObject::LoadImage(m_Images, IDB_ME,RGB(255,255,255),100,56,1);
	return true;
}
BOOL CMyPlane::Draw(CDC* pDC,BOOL bPause)
{
	m_nWait++;
	if(m_nWait>4)
		m_nWait=0;


	if(!bPause)
	{
		m_ptPos.x = m_ptPos.x + m_nHorMotion*8;
		m_ptPos.y = m_ptPos.y - m_nVerMotion*8;
	}

	if(m_ptPos.x>=GAME_WIDTH-PLANE_WIDTH)
		m_ptPos.x =GAME_WIDTH-PLANE_WIDTH;
	if(m_ptPos.x<=0)
		m_ptPos.x=0;

	if(m_ptPos.y<=0)
		m_ptPos.y =0;

	if(m_ptPos.y>=GAME_HEIGHT-PLANE_HEIGHT)
		m_ptPos.y=GAME_HEIGHT-PLANE_HEIGHT;


	m_Images.Draw(pDC,0,m_ptPos,ILD_TRANSPARENT);
	return TRUE;
}